...
export default function Dashboard() {
  ...
  const alCount = [...pagosSemana, ...arrastre].filter(p => getAlert(p, data.alertConfig) === 'red').length

  // Datos del gráfico — agrupar por día y tipo
  const chartData = useMemo(() => {
    return DIAS.map((dia, i) => {
      ...
      return {
        ...
        Echeq: echeq,
        total: efectivo + transferencia + echeq,
        pagos: pagosDelDia,
      }
    })
  }, [pagosSemana, lb])
  ...
  const toggleTipo = (tipo) => {
    setActiveTipos(prev => ({ ...prev, [tipo]: !prev[tipo] }))
  }
  ...
  const deletePago = (id) => {
    update(d => ({
      ...
      _pagoDeleted: id,
      _pagoChanged: null, _proveedorChanged: null, _proveedorDeleted: null,
      _ingresoChanged: null, _ingresoDeleted: null, _configChanged: null,
    }))
  }
  ...
  return (
    <div>
      ...
      <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 'var(--r)', padding: '20px 20px 10px', marginBottom: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 16, marginBottom: 16, alignItems: 'center' }}>
          ...
          {selectedDay && (
            <div style={{ marginLeft: 'auto', fontSize: 11, color: 'var(--text3)' }}>
              Click en la barra para cerrar el detalle
            </div>
          )}
        </div>
        ...
        {/* Fecha debajo de cada dia */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: 4, marginTop: 4, paddingLeft: 70, paddingRight: 10 }}>
          {chartData.map((d, i) => (
            <div key={i} style={{ textAlign: 'center', fontSize: 10, color: 'var(--text3)' }}>
      ...
      {selectedDay && pagosDelDiaSeleccionado.length > 0 && (
        <div style={{ background: 'var(--bg2)', border: '1px solid var(--accent)', borderRadius: 'var(--r)', padding: 18, marginBottom: 16 }}>
          <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 14, color: 'var(--accent)', display: 'flex', alignItems: 'center', gap: 8 }}>
            Pagos del {selectedDay} — {fDate(chartData.find(d => d.dia === selectedDay)?.fecha)}
      ...
      {selectedDay && pagosDelDiaSeleccionado.length === 0 && (
        <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 'var(--r)', padding: 20, marginBottom: 16, textAlign: 'center', color: 'var(--text3)', fontSize: 13 }}>
          Sin pagos pendientes el {selectedDay}
        </div>
      )}
